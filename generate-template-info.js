  ...
  templateDirs.forEach(dir => {
    ...
    if (fs.existsSync(packageJsonPath)) {
      try {
        ...
        templateInfo.name = packageJson.name || dir;
        ...
        if (deps) {
          Object.keys(deps).forEach(dep => {
            if (!templateInfo.tags.includes(dep)) {
              // 根据依赖名称添加相关标签
              if (dep.includes('vue')) {
                addUniqueTag(templateInfo.tags, 'vue');
              }
              if (dep.includes('react')) {
                addUniqueTag(templateInfo.tags, 'react');
              }
              if (dep.includes('angular')) {
                addUniqueTag(templateInfo.tags, 'angular');
              }
              if (dep.includes('typescript') || dep.includes('ts')) {
                addUniqueTag(templateInfo.tags, 'typescript');
              }
              if (dep.includes('sass') || dep.includes('scss')) {
                addUniqueTag(templateInfo.tags, 'scss');
              }
            }
          });
        }
