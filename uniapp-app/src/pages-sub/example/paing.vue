...
<template>
  <view class="h-full">
    <z-paging
      ref="pagingRef" v-model="dataList" class="flex-1" :refresher-enabled="true" :refresher-threshold="80"
      refresher-default-text="下拉刷新数据" refresher-pulling-text="释放立即刷新" refresher-refreshing-text="正在刷新..."
      refresher-complete-text="刷新完成" :loading-more-enabled="true" :loading-more-text="['点击加载更多', '正在加载...', '已全部加载']"
      empty-view-text="暂无内容数据" :auto-show-back-to-top="true" :back-to-top-threshold="300" @query="onQuery"
    ...
    >
            ...
            <view class="flex items-center justify-between">
              <view class="flex items-center space-x-4">
                <view class="flex items-center text-gray-500 space-x-1">
                  <view class="i-carbon-view h-4 w-4" />
                  <text class="text-sm">
                ...
                <button
                  ...
                  :class="item.isLiked ? 'text-red-500' : 'text-gray-500'" @click.stop="toggleLike(item)"
                >
                  <view :class="item.isLiked ? 'i-carbon-favorite-filled' : 'i-carbon-favorite'" class="h-4 w-4" />
                  <text class="text-sm">
                ...
                <view class="flex items-center text-gray-500 space-x-1">
                  <view class="i-carbon-chat h-4 w-4" />
                  <text class="text-sm">
              ...
              <view class="flex items-center space-x-2">
                <button class="p-2 text-gray-400 transition-all active:scale-95 hover:text-gray-600">
                  <view class="i-carbon-share h-4 w-4" />
                </button>
                <button class="p-2 text-gray-400 transition-all active:scale-95 hover:text-gray-600">
                  <view class="i-carbon-overflow-menu-horizontal h-4 w-4" />
                </button>
              </view>
            </view>
          </view>
      ...
      <template #empty>
        <view class="flex flex-col items-center justify-center py-16">
          <view class="i-carbon-document-blank mb-4 h-16 w-16 text-gray-300" />
